...
def load_song_with_browser(url: str) -> dict:
    try:
        from playwright.sync_api import sync_playwright
    except ImportError as exc:
        raise RuntimeError(
...
def extract_store(html_text: str) -> dict:
    if UG_BLOCKED_RE.search(html_text):
        raise ValueError(
            "Ultimate Guitar returned a Cloudflare challenge and the browser fallback did not reach the page data"
        )
    ...
    return json.loads(html.unescape(parser.data_content))


def find_tab_view(node):
...
def extract_song(store: dict) -> dict:
...
def extract_title(text: str, page_title: str) -> str:
...
def extract_artist(text: str, page_title: str) -> str:
    match = TITLE_RE.search(text)
    if match:
